...
function trace() {
  if (!trace_broker) trace_broker = new TraceBroker();
  return trace_broker;
}
...
class TraceBroker extends events.EventEmitter {
  ...
  constructor() {
    ...
    this.trace_native = require('bindings')('trace_native');
  ...
  enableNativeConsoleListener() {
    this.trace_native.enableNativeConsoleListener();
  }
